...
export class LLMService {
    ...
    async parseResumeText(text) {
        ...
        const messages = [
            {
                role: 'system',
                content: `你是专业的简历解析助手。你的任务是从简历原始文本中提取结构化信息。

【重要规则】
1. 必须严格按JSON格式输出，不要有任何额外文字
2. 字段值必须准确，不要编造不存在的信息
3. 如果某个字段无法识别，填空字符串""或空数组[]
4. 所有字段名用英文，字段值用中文（原文语言）
5. 时间格式统一为 "YYYY.MM-YYYY.MM" 或 "YYYY.MM-至今"
6. 技能关键词保持原文中的表述
7. 描述字段保留原文关键信息，不超过200字`
            },
            {
                role: 'user',
                content: `请从以下简历文本中提取结构化信息，严格按JSON格式输出：
